...
**#3 复用判定验证** (2026-05-29): `hasPartial(role)` 仅检查
`checkpoint.partials[role]`，`mergeCheckpointError` 写入 `errors[role]`
不碰 `partials`。失败 Agent 不会被复用——重试时必定重新调 LLM。

**#4 SW 终止 → UI 失败态验证** (2026-05-29): content.js 三条错误路径
（`!resp` → "service worker 无响应"、`!resp.ok` → error 文本、
`catch` → "通信错误"）全部以 `setBody('<div class="error">...')` 终止，
`finally` 块隐藏 loading 并重置 busy。SW 死亡不会导致无限 spinner，用户
看到可重试的错误提示。

**相关文件**: `lib/agents/runner.js`、`background.js:274-280,504-507`
**测试**: `test/agents/runner.test.js` — 7 个 checkpoint 专项测试（含 2
个指纹专项 + 1 个"错误不复用"验证）
...
### P1 Eval 特征化审计（2026-05-29）

对上述边际收益表的独立审计结论（`docs/p1-eval-characterization.md`）：

1. **GT**: 6 个月 alpha（vs 沪深300），5 级离散化（阈值为 ±3%/±10%）
2. **Score**: 加权命中率矩阵，neutral 有结构性 0.3 保底——"永远 neutral"得分 0.401，碾压所有模型
3. **样本**: 40 只股票 × 4 时点 × 4 模板 = 640 条，有效独立样本 ~160，误差棒 ±0.08
4. **泄漏**: MA/MACD 计算无前视泄漏；但所有 prompt 版本共享同一 frozen dataset = 过拟合风险
5. **Null 基线**: always neutral=0.401, random=0.144, frozen baseline=0.197, v4=0.168

**关键认知修正**: 0.187 不是"质变"，只是在一个对 neutral 有结构性优势的评分函数下、从极小样本测得的小幅高于随机的分数。±0.08 的误差棒下无法与 random 区分。

### P1 条件信号 & LSTM 泄漏审计（2026-05-29）

**条件信号分析**（`docs/p1-conditional-signal.md`）— 完全不用评分矩阵，用真 alpha：
- strong_bull 桶 alpha=18-20%，远高于无条件 6%——模型能识别极端强势股
- bull/bear/neutral/strong_bear 四桶之间无显著区分（strong_bear 桶 alpha=+9.9%，为正！）
- spread (bullish − bearish) = 7.5%，95% CI = [-2.2%, +19.8%]，**含 0 → 不显著**
- 方向命中率 58.9%，仅比 always-bullish (50.7%) 高 +8pp
- v4 (0.187) vs baseline (0.197) 在 alpha 空间**完全无差异**

**LSTM 泄漏检查**（`docs/p1-lstm-leak-check.md`）:
- `daily_lstm7.pt` 训练数据无时间切分（`scripts/daily_to_monthly_aggr.py:64`）→ 2024+ 全在训练集
- 所有 eval 时点 (2024-05~2025-11) 在训练数据内 → **mc-dropout eval 全部作废**
- v4 eval (0.187) 未注入 LSTM → 不受影响
- **运行时 LSTM 信号已默认关闭** (`p1-disable-leaked-lstm`, 2026-05-29): `ENABLE_LSTM_SIGNAL = false`
  走与 native host 不可用相同的 null 降级路径
- 待 walk-forward 重训 + 严格时间切分验证 IC 跑赢基线后再重启

**strong_bull vs 动量基线**（`docs/p1-strongbull-validation.md`）:
- 判读 C — unique-pair 层级 CI 下界 5.02% < 无条件 6.07%, strong_bull 自身不显著
- 动量基线 alpha=12.7%, Jaccard=0.31, 差值 CI 含 0
- 结论: 34 独立样本不够，必须先扩样本；失效不是模型问题而是样本量问题

**功效分析**（`docs/p1-power-analysis.md`）:
- alpha std=33.7%, MDE(现状)=31.5% — 只能检出 >37.6% 的效应
- 当前点估计 12.5% 远在 MDE 之下 → 前面所有"不显著"是 N 不够，非模型无能
- Winsorize std→20%, 效应≥6% → 需 ~330 strong_bull ≈ 1660 pairs ≈ 280只×6时点
- 推荐路径: Winsorize + 100 只 × 12 时点 (1200 pairs, ~240 sb)

### P1 Eval 重建设计（2026-05-29）

**`docs/p1-eval-rebuild-design.md`** — UNIVERSE=low_position, 退役评分矩阵, 重建 eval：

- **低位口径**: close 处于过去 12 月区间底部 20% AND close < MA60
- **时点**: 12 个跨 4 种 regime (2018-06 ~ 2024-10), 6 Train + 6 Test (hold-out)
- **双尺子**: 稳健中心 (Winsorize spread + median) + 尾部捕获 (top-20% capture, >20% bull hit)
- **反转因子基线**: 短期反转 + MA60 折扣 + RSI 超卖 (禁用动量——低位股动量一律为负)
- **胜负手**: LLM spread 能否跑赢反转因子 spread
