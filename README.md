...
- 🚩 **[2026.06] Please use the latest commit — earlier versions carry inflated figures, since corrected.** If you cloned this repo before mid-June 2026, run `git pull`. What was wrong in older versions and is now fixed:
  - **Inflated Sharpe from √52 weekly annualization.** An earlier News entry advertised a margin-account Sharpe of ~3.1. That used weekly-return √52 annualization, which is inflated ~15–25% by positive autocorrelation. On the honest non-overlapping monthly (√12) basis: **pure-cash retail ≈ 2.0–2.2**, margin/short long-short leg ≈ **2.5–2.7**. The ~3.1 figure is withdrawn — do not cite it.
  - **Over-stated margin/short combo (non-borrowable short leg).** An intermediate write-up put the neutralized long-short combo at 2.94. That short leg included illiquid names you cannot actually borrow to short. Under a realistic *marginable-only* constraint (short leg restricted to the liquidity top 50%), the honest deployable figure is **2.88** — it still clears 2.8, but 2.94 was an over-statement and should not be quoted.
  - **Factor pool now frozen (pre-registration).** The exact factor set behind the 2.88 result is locked in `docs/koc-factor-pool-lock.md` to prevent factor-snooping. Treat only locked-spec results as trustworthy; any number produced before the freeze could in principle be affected by selection.
  - **Caveat unchanged:** even 2.88 requires a **securities-lending (leverage) account that ordinary retail cannot access**, and statistical confidence over the 5-year sample is only ~45% (bootstrap P(Sharpe>2.8)). **This is not a target to chase.** The shipped, retail-accessible product remains the pure-cash ≈ 2.0–2.2 strategy.
- 🚩 **[2026.06] Pure-cash retail strategy shipped.** A weekly signal generator for an ordinary cash account (no securities lending, no futures, no options): convertible-bond double-low + IPO subscription + low-vol/dividend base position + reverse-repo for idle cash. Honest **back-tested** net Sharpe ≈ **2.0–2.2** (non-overlapping monthly, deflated; net of estimated transaction costs, before real-world slippage). **Back-tested performance does not indicate future results.** **Ships with a blunt disclaimer: this is an experimental research tool, not a money-making system — do not use leverage, do not touch options, and do not expect to reliably profit from the market (most retail traders lose over time).** Entry point `scripts/koc/retail_signal.py`; core `lib/portfolio/retail_cash.py`; 41 unit tests; reviewed via an external LLM. Follow-up improvements are still being considered.
  - **On the margin (融券 / securities-lending) version — ⚠️ NOT shipped, NOT for retail:** securities lending is leverage that ordinary retail cannot practically access, and the disclaimer above applies doubly — **this is not a target to chase.** Disclosed only for honesty: adding a market-neutral long-short leg raises the **back-tested** deflated Sharpe to ≈ **2.5–2.7**, still within the realized net range of professional Chinese quant funds (~1.4–2.4).
- 🚩 **[2026.06] Correction — the earlier "Sharpe ≈ 3.1" claim was not achieved.** Re-audit found those figures used weekly-return √52 annualization, which is **inflated ~15-25% by positive autocorrelation** (convertible holdings trend; IPO returns are lumpy). On the honest non-overlapping monthly basis, deflated for multiple-testing across 1300+ configurations:
  - **Pure-cash retail (no securities lending, no futures, no options — what ordinary retail can actually access): honest net Sharpe ≈ 2.0–2.2.** This is the productized strategy (convertible double-low + IPO subscription + low-vol/dividend base position + reverse-repo). See `scripts/koc/retail_signal.py`.
  - Margin/short version (market-neutral stock long-short) reaches ~2.5–2.7 deflated, but **requires securities lending that ordinary retail cannot access** — so it is not the shipped product.
  - **Sharpe 3 is above the realized net-of-fee ceiling of essentially the entire Chinese 私募 industry (~2.5).** No third orthogonal Sharpe≥1.5 cash-only stream exists (52-factor zoo + two independent reviews confirm). Honesty over marketing.
