...
- 🚩 **[2026.06] Retail strategy — honest result (corrects an earlier "Sharpe ≈ 3.1" preview that was NOT achieved).** The 3.1 figure used weekly √52 annualization, **inflated ~15-25% by autocorrelation** (convertible holdings trend; IPO returns are lumpy). On the honest non-overlapping monthly basis, deflated for multiple-testing across 1300+ configurations: **pure-cash retail (no leverage, no options, no securities lending — what ordinary retail can actually access): back-tested net Sharpe ≈ 2.0–2.2.** A margin/short variant reaches ~2.5–2.7 but requires securities lending ordinary retail cannot access — deliberately not shipped. **Sharpe 3 is above the realized net-of-fee ceiling of essentially the entire Chinese 私募 industry (~2.5).** This is an experimental research tool — do not use leverage or options, and do not expect to reliably profit (most retail traders lose over time). Productized signal generator + full audit on branch `pead-koc-sue`. Honesty over marketing.
