...
// 1 unique call per (stock, cutoffDate)
...
const RUN_ID = `lowpos-${new Date().toISOString().slice(0,19).replace(/[T:]/g,'-')}`;
...
async function callLLM(prompt) {
  ...
  return { text: d.choices?.[0]?.message?.content || '', usage: { inputTokens: d.usage?.prompt_tokens || 0, outputTokens: d.usage?.completion_tokens || 0 } };
}
...
for (let i=0;i<pending.length;i+=2) {
  ...
  const results=await Promise.allSettled(batch.map(async tp=>{
    const {text,usage}=await retry(buildPrompt(tp));
    return {stockCode:tp.stockCode,cutoffDate:tp.cutoffDate,signal:parseSignal(text),alpha:tp.alpha,cost:usage.inputTokens/1e6*1+usage.outputTokens/1e6*4,tokens:usage,timestamp:new Date().toISOString()};
  }));
  for(const r of results){
    if(r.status==='fulfilled'){appendFileSync(OUT_PATH,JSON.stringify(r.value)+'\n');totalCost+=r.value.cost;ok++;}
    else{fail++;console.warn('FAIL:',String(r.reason).slice(0,100));}
  }
  if((ok+fail)%20===0||ok+fail===pairs.length) console.log(` ${ok+fail}/${pairs.length} ok=${ok} ¥${totalCost.toFixed(2)} ${((Date.now()-start)/60000).toFixed(1)}min`);
  if(i+2<pending.length) await new Promise(r=>setTimeout(r,500));
}
