...
// 输出语言指令（outputLang='en' 时追加）。分析约束保持中文（缠论等术语的语义资产），
// 仅切换报告正文语言；结构化 JSON 块的字段与取值不受影响（解析契约不变）。
const OUTPUT_LANG_EN_BLOCK = `## 输出语言要求

最终分析报告正文请全部用英文撰写（Write the final report in English）。技术术语首次出现时可用括号附注中文原文，例如 "central zone (中枢)"。
注意：报告末尾的结构化 JSON 块不受本条影响——字段名与取值必须严格保持模板规定的原样（含中文取值），不要翻译，供内部工具解析。`;

export { OUTPUT_LANG_EN_BLOCK };
 ...
 */
export async function buildPromptByTemplate({ templateKey = DEFAULT_TEMPLATE, name, code, market, klines, period = 'monthly', provider = 'anthropic', extraContext, decisionMode = false, indexData = null, resonance = null, sectorAlphaData = null, lstmSignalData = null, kronosSignalData = null, outputLang = 'zh' }) {
...
用 Markdown 格式输出。`;
  ...
  prompt += '\n' + validationWarning;
  if (outputLang === 'en') prompt += '\n\n' + OUTPUT_LANG_EN_BLOCK;
...
export function buildPrompt({ name, code, klines, style = 'technical', period = 'monthly', provider = 'anthropic', extraContext, decisionMode = false, indexData = null, outputLang = 'zh' }) {
  if (!Array.isArray(klines) || klines.length === 0) {
    throw new Error('klines 为空,无法构造 prompt');
  }
