...
/**
 * @param {object} params
 * @param {string} params.market - '0'(SZ) / '1'(SH)
...
export async function fetchKlines(params = {}) {
  ...
  // start_time: when limit is 0 or very large, fetch all (back to 1990); otherwise estimate from limit
  const now = new Date();
  ...
  // marketData is semicolon-delimited rows, each row is comma-separated fields
  // headers: [timestamp, time, open, close, volume, high, low, amount, change, changePercent, turnoverRate, prevClose, ma5Price, ma5Volume, ma10Price, ma10Volume, ma20Price, ma20Volume]
  const rows = raw.marketData.split(';').filter(Boolean);
  const klines = [];

  for (const row of rows) {
    const fields = row.split(',');
    const dateStr = fields[1]; // Raw format: 2024-01-31
    if (!dateStr) continue;
  ...
  // limit=0 or very large → return all; otherwise take last N bars
