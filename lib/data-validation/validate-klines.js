...
export function checkKlines(klines, period = 'monthly') {
  ...
  // Deduplicate (limit same-type issues to 5)
  const limited = [];
  const counter = {};
...
/** Calculate period gap between two dates; returns null if unable to determine */
function dateGap(prev, curr, period) {
  ...
  // daily/weekly: simple date string comparison
  if (prev < curr) return 0; // As long as dates are increasing, don't check exact gap
  return null;
}

/** Generate warning text injectable into prompts */
