...
export function buildCalibrator(evaluations) {
  ...
  // Group by predictedSignal, count actual groundTruth distribution
  const groups = {};
 ...
 * @param {object} calibrator — returned by buildCalibrator
 * @returns {object} — { signal: calibrated signal, rawSignal: original, confidence: 'calibrated' }
 */
export function calibrateOutput(rawOutput, calibrator) {
  ...
  // Use distribution mode as calibrated signal
  ...
  // If the most probable groundTruth < 30%, don't flip (uncertain)
  if (bestProb < 0.30) {
    return { ...rawOutput, calibrated: false };
  }
...
/**
 * Batch calibrate eval results, return new results array
 */
...
// Avoid circular dependency, inline scorePrediction
