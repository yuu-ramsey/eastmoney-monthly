 ...
 * @param {Array} records — parsed eval jsonl record array
...
/**
 * Format comparison output (for CLI reports)
 */
export function formatScoreComparison(result) {
  const { full, exclPf } = result;
  const lines = [
    '=== Score Calculation (transparent denominator) ===',
    '',
    `Full denominator: weightedScore=${full.weightedScore}, n=${full.denominator}`,
    `Excl PF:   weightedScore=${exclPf.weightedScore}, n=${exclPf.denominator}`,
    `PF count:  ${full.signalDistribution.parse_failed}/${full.denominator}`,
    '',
    'Warning: Report uses full.weightedScore as the official score. exclPf is for reference only.',
  ];
  return lines.join('\n');
}
