...
export async function buildDataset(stockList, options = {}) {
  ...
  for (const stock of stockList) {
    ...
    // 可选 cutoff 范围：从 earliestMonthsAgo 到 horizon 前
    const now = new Date();
    ...
    // 找 klines 中在 earliestDate ~ latestDate 之间的索引
    const candidates = [];
    ...
    // 均匀采样 testPointsPerStock 个，保证间距 ≥ minDateGap 天
    const selected = [];
