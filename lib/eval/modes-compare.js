...
export async function runModesCompare(v1RunId) {
  ...
  const dataset = fs.existsSync(datasetPath)
    ...
    : { stocks: [], testPoints: [] };
  // Fill category+market from seed-stocks
  ...
  // Add quant score to each v1 result
  const enriched = [];
  for (const r of v1Results) {
    ...
    // Use recent 24 klines (simulate visible data at cutoff)
    // Since cutoffIndex info is in dataset, use -24 as approximation here
    ...
    // LLM result (reconstructed from v1 eval)
    const llmResult = {
    ...
    // Three modes
    // mode 1: llm_only = original v1 result
  ...
  // Regime distribution (adaptive mode only)
  console.log(`\n=== Adaptive Regime Distribution ===`);
  const adaptiveResults = enriched.filter(r => r.mode === 'adaptive' && !r.error);
  console.log(`| Category | total | strong_trend% | sideways% | high_vol% | mixed% |`);
  console.log(`|------|-------|--------------|-----------|-----------|--------|`);
