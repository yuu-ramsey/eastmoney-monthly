 ...
 * @returns {{
 *   icSeries: {date: string, ic: number, n: number, grossSpread: number}[],
 *   neutralizationStatus: 'full'|'partial'|'none',
 *   controlsUsed: string[]
 * }}
 */
function computeICSeries(allRecords, signalFn, options = {}) {
  const {
    ...
    alphaKey = 'alpha',
    neutralizeOptions = {},
    holdingPeriod = null,  // reserved for overlap check
  } = options;

  const groups = groupByDate(allRecords, signalFn);
  const icSeries = [];

  // Track neutralization quality across all cross-sections
  ...
  for (const [date, recs] of [...groups.entries()].sort()) {
    ...
    if (doNeutralize) {
      const neutralResult = neutralizeCrossSection(
        filteredRecords,
        r => signalFn(r),
        { ...neutralizeOptions, cutoffDate: date }
      );
      const { residuals, neutralizationStatus: ns, controlsUsed: cu } = neutralResult;
      if (ns === 'partial') anyPartial = true;
      cu.forEach(c => allControlsUsed.add(c));
  ...
  const neutralizationStatus = !doNeutralize ? 'none' : anyPartial ? 'partial' : 'full';
  return { icSeries, neutralizationStatus, controlsUsed: [...allControlsUsed] };
}
 ...
 *   metrics: {
 *     icSeries: object[], icMean: number, icStd: number, icir: number, tStat: number,
 *     neutralizationStatus: 'full'|'partial'|'none', controlsUsed: string[]
 *   },
 *   verdict: 'pass' | 'fail',
...
function validateSignal(allRecords, signalFn, options = {}) {
  ...
  const { icSeries, neutralizationStatus, controlsUsed } =
    computeICSeries(allRecords, signalFn, validateOptions);
  ...
  if (icSeries.length === 0) {
    return {
      ...
      metrics: { icSeries: [], icMean: 0, icStd: 0, icir: 0, tStat: 0, neutralizationStatus, controlsUsed },
      verdict: 'fail',
      sanityWarnings: [],
    };
  }

  // Warn downstream if neutralization is partial (DATA_PENDING)
  if (neutralizationStatus === 'partial') {
    console.warn(
      `  WARN validateSignal: neutralizationStatus='partial' — ` +
      `size/liquidity DATA_PENDING. Results are NOT final until §15/§16 data is available.`
    );
  }
  ...
  return {
    ...
    metrics: { icSeries, icMean, icStd, icir, tStat, nTimepoints: icVals.length, neutralizationStatus, controlsUsed },
    verdict,
    sanityWarnings,
  };
}
