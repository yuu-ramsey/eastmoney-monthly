...
/**
 * Extract directional judgment from analysis text
...
export function extractJudgment(text) {
  ...
  // Markdown bold: **Direction Judgment**: **Neutral/Range-bound (slightly bearish)**
  const md = text.match(/\*\*方向判断\*\*[：:]\s*\*\*([^*]+)\*\*/);
  if (md) return parseDirection(md[1]);

  // Direction Judgment: 【bullish】 format
...
/**
 * Batch evaluation with incremental writes
 ...
 * @returns {Array} Newly added evaluation records
 */
