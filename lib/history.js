// Analysis history pure functions (no chrome.storage dependency)
...
export function historyToMarkdown(entry) {
  ...
  const templateLabels = { technical: 'Technical', trend: 'Trend', valuation: 'Valuation', sentiment: 'Sentiment' };
...
/**
 * Check if history list needs trimming
...
export function checkCapacity(list, maxItems = MAX_HISTORY_ITEMS, maxBytes = MAX_HISTORY_BYTES) {
  if (list.length > maxItems) {
    return { trimmed: true, reason: `Count exceeded (${list.length} > ${maxItems})` };
  }
  let bytes;
  try {
  ...
  if (bytes > maxBytes) {
    return { trimmed: true, reason: `Size exceeded (${(bytes / 1024 / 1024).toFixed(1)}MB > ${(maxBytes / 1024 / 1024).toFixed(1)}MB)` };
  }
