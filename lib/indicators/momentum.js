...
/** TDX RSI(N)
 *  LC = REF(CLOSE,1), SMA of MAX(CLOSE-LC,0) / ABS(CLOSE-LC) with (N,1) * 100
 */
 ...
 *  K = SMA(RSV, M1, 1)  // i.e. 1/M1*RSV + (M1-1)/M1*prevK, init K=50
 *  D = SMA(K, M2, 1)
 *  J = 3K - 2D
 */
...
/** Williams %R(N) = (HHV(N) - C) / (HHV(N) - LLV(N)) * 100 */
...
/** American-style Stochastic (Fast) — %K: (C - LLV) / (HHV - LLV) * 100, %D: SMA(%K, d) */
