...
/**
 * Calculate single-window return + volatility
 * @param {number[]} closes
...
function calcWindowRatio(closes, windowLen, periodsPerYear) {
  ...
  // Rolling window returns, used to compute volatility (take most recent min(n-windowLen, periodsPerYear) windows)
  const rollingRets = [];
...
/**
 * Calculate current ratio's percentile rank among all historical windows
 * @param {number[]} closes
 * @param {number} windowLen
...
/**
 * Main entry: calculate multi-window normalized returns
 * @param {Array} klines — kline array [{ close, ... }]
...
export function calcNormalizedReturns(klines, period) {
  ...
  // Find the window with best risk/reward ratio
  const best = windows.reduce((a, b) => (b.ratio > a.ratio ? b : a), windows[0]);

  // Trend: ratio series of the shortest window over recent periods
