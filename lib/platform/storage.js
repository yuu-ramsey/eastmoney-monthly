...
// ---- Node file implementation (lazy init) ----
...
async function _ensureNodeModules() {
  if (_fs) return;
  // Dynamic imports only in Node env, browser never reaches here
...
async function getFromFile(keys) {
  ...
  if (keys === null) {
    ...
    } catch (_) { /* directory does not exist */ }
    return result;
  }
  ...
  for (const k of keyList) {
    ...
    } catch (_) { /* file not found treated as undefined */ }
  }
  return result;
}
