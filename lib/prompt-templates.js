...
// [已废弃] 反转因子信号 — 24tp 池翻负 -19.2%，已从 runtime 撤出。保留空函数防止 import 报错。
export function buildReversalSignalBlock(_signalData) {
  return '';
}

// Kronos 月线预测信号 — 24tp 唯一通过门控的方向信号（docs/p3-kronos-confirm.md, docs/p3-llm-gate-24tp.md）
export function buildKronosSignalBlock(signalData) {
  ...
  const sign = pct > 0 ? '+' : '';
  return `
## Kronos 月线预测信号（24tp hold-out 唯一通过门控的方向信号）

- 预测 6 个月方向：**${dir}**（预测变动 ${sign}${pct.toFixed(1)}%）
- 验证数据：无偏低位池 24 时点, test spread +9.7% CI[+5.1,+15.0], decay=0.80
- LLM 技术分析 24tp 方向信号 +1.6% CI[-3.4,+6.6] 含 0（不显著）→ Kronos 是唯一可靠方向信号

**使用指引**：Kronos 是唯一通过 24tp hold-out 验证的方向信号。LLM 技术分析为辅助解读。若 Kronos 与技术面分歧，优先参考 Kronos 方向，用技术面解释可能原因。
`;
}
