...
11. **多周期共振约束**（基于历史回测：75% 假阳性来自周期背离）：
   - strong_bull 触发条件：共振 dominant = bull 且（strong 共振 OR partial共振 + 信号指引 strong_bull）
   - strong_bear 触发条件：共振 dominant = bear 且（strong 共振 OR partial共振 + 信号指引 strong_bear）
   - partial 共振（2/3 同向）：信号指引触发时可判 strong，否则最多判 bull/bear
   - divergent（无共振）：应判 neutral
   - 违反共振约束判 strong = 失职。历史数据证实周期背离时判 strong 有 75% 概率错误
