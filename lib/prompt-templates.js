   ...
   - 违反此约束（strong_bull 共振时判 strong_bull）= 失职。历史证实三周期全偏多是最不可靠的看多信号
	...
	12. **行业相对强弱强制推理**：若上方存在"HS300 行业内 Alpha"段，你必须严格执行以下推理流程：
		...
		**推理约束**：
		...
		14. **LSTM 量化信号约束**（若上方存在"LSTM 日线预测信号"段）：
		该信号来自日线 LSTM 模型（Test IC=0.10，经验证有效）。推理规则：
		- LSTM signal > +0.5 → 量化看多（你的 signal 若与此相反，必须给出技术面反驳理由）
		- -0.5 ≤ signal ≤ +0.5 → 量化中性（LSTM 无明确方向，按技术面自由判断）
		- signal < -0.5 → 量化看空（你的 signal 若与此相反，必须给出技术面反驳理由）
		注意：LSTM 信号不与你的技术分析绑定——你可以合理反驳，但不能无视。
`;


// 反转因子信号（已验证 hold-out）— 接入 docs/p3-signal-gating.md
export function buildReversalSignalBlock(signalData) {
  if (!signalData || signalData.composite == null) return '';
  const comp = signalData.composite;
  const dir = comp > 0.2 ? 'bullish' : comp < -0.2 ? 'bearish' : 'neutral';
  return `
## 反转因子信号（已通过 hold-out 验证）

- 反转复合分：${comp.toFixed(3)}（rev1m + rev3m + ma60_discount）
- 方向推断：**${dir}**
- 验证数据：无偏低位池 test spread +6.6% CI[+1.3,+11.2], decay 健康

**使用指引**：反转因子是独立的"越跌越买"信号——回踩支撑的低位股往往反弹。若与 Kronos 或你的技术判断同向则加强；若反向，以技术面+基本面的综合判断为准。
`;
}

// Kronos 月线预测信号（已验证 hold-out）— 接入 docs/p3-kronos-confirm.md
export function buildKronosSignalBlock(signalData) {
  ...
  const sign = pct > 0 ? '+' : '';
  return `
## Kronos 月线预测信号（已通过 hold-out 验证）

- 预测 6 个月方向：**${dir}**（预测变动 ${sign}${pct.toFixed(1)}%）
- 验证数据：无偏低位池 24 时点, test spread +7.6% CI[+5.1,+15.0], decay=0.80
- 与 LLM 技术分析零相关（r=0.069）→ 互补信号

**使用指引**：Kronos 是月线 OHLCV 形态识别模型。若与你的技术判断一致，可标注"Kronos 同向"；若反向，以你的技术判断为准，注明"Kronos 反向"即可。
`;
}

// HS300 行业内 Alpha 段落格式化
// LSTM 日线预测信号块（含 MC Dropout 不确定性）
export function buildLstmSignalBlock(signalData) {
  ...
  if (signalData.uncertainty_level) {
    ...
    const direction = sig > 0.01 ? '看多' : sig < -0.01 ? '看空' : '中性';
    const sign = sigRaw > 0 ? '+' : '';
    ...
    return `
## LSTM 量化预测信号（含 MC Dropout 不确定性）

| 指标 | 数值 |
|------|------|
| 原始预测 y3 | ${sign}${sigRaw.toFixed(4)} |
| 不确定性 σ | ${signalData.y3_std.toFixed(4)} |
| 置信度惩罚后 y3 | ${sig >= 0 ? '+' : ''}${sig.toFixed(4)} |
| 预测置信度 | ${confPct} |
| 不确定性等级 | ${signalData.uncertainty_emoji} **${signalData.uncertainty_level === 'low' ? '低' : signalData.uncertainty_level === 'medium' ? '中' : '高'}不确定性** |
| 远期预测 y6 | ${signalData.y6_mean != null ? (signalData.y6_mean >= 0 ? '+' : '') + signalData.y6_mean.toFixed(4) + ' ± ' + signalData.y6_std.toFixed(4) : 'N/A'} |
| MC 采样次数 | ${signalData.mc_samples || 50} 次 |

${signalData.uncertainty_desc}

**使用指引**：
${signalData.uncertainty_level === 'low'
    ? '- 模型信号可信度较高，可作为技术分析的主要量化参考。若技术面与此同向，置信度上调；若反向，需给出明确技术面反驳理由。'
    : signalData.uncertainty_level === 'medium'
    ? '- 模型信号存在分歧，仅作辅助参考。你的技术分析判断权重应高于量化信号。若技术面与此反向，以技术面为准。'
    : '- 模型信号分歧大，不可靠。以你的技术分析为唯一判断依据，此信号仅记录备用。'}
- 不要直接复制此信号作为你的结论，必须结合技术面独立判断。
`;
  }

  // 旧格式兼容：仅标量信号
  const sig = signalData.lstm_signal;
  const strength = sig > 0.5 ? '看多' : sig < -0.5 ? '看空' : '中性';
  const sign = sig > 0 ? '+' : '';
  return `
## LSTM 日线预测信号（量化模型）
...
| 指标 | 数值 |
|------|------|
| LSTM 信号 | ${sign}${sig.toFixed(4)} |
| 方向 | **${strength}** |

该信号来自量化模型，无不确定性估计。在你的分析中引用此信号方向，但以技术面判断为准。
`;
}
...
性价比 = 窗口收益率 / 年化波动率。跨窗口可直接比较，正值越大性价比越高。

| 时间窗口 | 收益率 | 年化波动 | 性价比 | 历史分位 |
|----------|--------|----------|--------|----------|
${rows}

**最佳窗口**：${nrData.bestWindow}，综合趋势：**${trendLabel}**

**使用指引**：
- 性价比是跨周期可比的"风险调整后回报"，不等于方向判断
- 性价比为正且分位 > P70 的窗口 → 该时间尺度上风险回报比有利，可作为仓位时间锚定参考
- 性价比趋势"上升"→ 近期风险调整后回报改善，可适度上调整体信心
- 性价比趋势"下降"→ 近期风险调整后回报恶化，综合结论中应体现审慎
- 此数据仅供决策参考，不直接决定买卖方向
`;
}
