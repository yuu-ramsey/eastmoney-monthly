...
// Signal strength keywords (Chinese technical analysis terms retained as data labels)
 ...
 * @returns {string} daily report path
 */
export function generateDailyReport(scanResults, meta = {}, date = new Date()) {
  const today = date.toISOString().slice(0, 10);

  // Scoring: prefer scoreData.score, fall back to keyword matching
  const scored = scanResults
    .filter((r) => !r.error)
    .map((r) => {
      const kw = scoreSignal(r);
      // If scoreData was parsed, use its score instead of keyword scoring
