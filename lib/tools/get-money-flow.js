...
export const getMoneyFlowTool = {
  ...
  async handler(input) {
    ...
    // Parse kline string (format: date,major_net_inflow,...)
    const rows = [];
    let totalInflow = 0;
    ...
    for (const line of klines) {
      ...
      const date = fields[0] || '';
      const mainInflow = parseFloat(fields[1]) || 0; // f52: major net inflow (yuan)
      const retailInflow = parseFloat(fields[2]) || 0; // f53: retail net inflow (yuan)
