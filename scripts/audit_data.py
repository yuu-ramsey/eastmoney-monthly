...
for code in codes:
    ...
    for i in range(60, n-6):  # i = signal computation index
        ...
        signal_date = dates[i]           # Signal calculation date (current month-end)
        ret_start_date = dates[i+1]      # Return start date (next month-end, used as c[i] in return = (c[i+3]-c[i])/c[i])
        ret_end_date = dates[i+3]        # Return end date (3 months later)
