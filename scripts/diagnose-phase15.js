...
// Pick 5 samples: 2-3 strong GT + 2-3 bull/bear GT
...
// Pick first entry with different stockCode + template for each sample
...
for (let i = 0; i < samples.length; i++) {
  ...
  console.log(`\n--- Agent Reference Stats ---`);
  console.log(`Judge references Bull: ${bullMentions}x | Bear: ${bearMentions}x | Technical: ${techMentions}x | Sector: ${sectorMentions}x`);
  ...
  console.log(`Bull first heading: ${bullFirst.trim()}`);
  console.log(`Bear first heading: ${bearFirst.trim()}`);
}
...
console.log(`Agent success rate: Bull=${allBull}/48 Bear=${allBear}/48 Tech=${allTech}/48 Sector=${allSector}/48`);

// Agent output average length
const avgLen = (arr) => arr.reduce((s, r) => s + (r?.length || 0), 0) / Math.max(1, arr.length);
console.log(`Average length: Bull=${Math.round(avgLen(records.map(r=>r.bullText)))} Bear=${Math.round(avgLen(records.map(r=>r.bearText)))} Tech=${Math.round(avgLen(records.map(r=>r.techText)))} Sector=${Math.round(avgLen(records.map(r=>r.sectorText)))} Judge=${Math.round(avgLen(records.map(r=>r.judgeText)))}`);
