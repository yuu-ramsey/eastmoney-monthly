# Download weekly K-lines for all stocks via Scrapling (Baidu API)
import sqlite3, time
...
weekly = set(r[0] for r in conn.execute('SELECT DISTINCT code FROM weekly_klines'))
missing = sorted(monthly - weekly)
print(f'Monthly: {len(monthly)}, Weekly: {len(weekly)}, Missing: {len(missing)}')
...
for i, code in enumerate(missing):
    ...
    url = (f'{API}?all=1&isIndex=false&isBk=false&isBlock=false'
           ...
           f'&code={code}&ktype=2&start_time=2010-01-01%2000:00:00')  # ktype=2 = weekly
    try:
        resp = f.get(url)
        data = resp.json()
        if data.get('ResultCode') != '0':
            err += 1
            if err <= 2: print(f'  {code} API err', flush=True)
            continue
        ...
        for row_str in rows_text:
            ...
            try:
                open_p=float(parts[2]); close_p=float(parts[3]); volume=float(parts[4])
                high_p=float(parts[5]); low_p=float(parts[6])
                ...
                rows_data.append((code,date_str,open_p,close_p,high_p,low_p,
                                  volume,amount,amplitude,change_pct,0,turnover,1.0,'baidu'))
            except (ValueError,IndexError): continue

        if rows_data:
            conn.executemany(
                'INSERT OR REPLACE INTO weekly_klines '
        ...
        err += 1
        if err <= 3: print(f'  {code} {type(e).__name__}', flush=True)
        time.sleep(1)

    if (i+1) % 100 == 0:
        elapsed = time.time()-t0
        eta = elapsed/(i+1)*(len(missing)-i-1)
        print(f'  [{i+1}/{len(missing)}] done={done} err={err} {elapsed:.0f}s ETA {eta:.0f}s', flush=True)
        time.sleep(1)
