# Full IC report: Daily / Weekly / Monthly (all frequencies, full data)
...
tables = {r[0]: conn.execute(f'SELECT COUNT(*) FROM {r[0]}').fetchone()[0]
          for r in conn.execute("SELECT name FROM sqlite_master WHERE type='table'").fetchall()}

def build_features(df, codes):
    ...
    flat_a = np.array(flat_list, dtype=np.float32)
    ys_a = np.array(ys_list, dtype=np.float32)
    dates_a = np.array(dates_list)
    v = ~np.isnan(flat_a).any(axis=1) & ~np.isnan(ys_a)
    return flat_a[v], ys_a[v], dates_a[v]

def train_eval(flat, ys, dates_arr, label):
...
for table, label in [('monthly_klines','Monthly'),('weekly_klines','Weekly'),('daily_klines','Daily')]:
    ...
    flat, ys, dates_arr = build_features(df, codes)
    print(f'  {len(codes)} stocks, {len(flat):,} samples, {flat.shape[1]} features', flush=True)
    ic, hit, ls, n_test, n_months, ics, elapsed, n_feat = train_eval(flat, ys, dates_arr, label)
    results[label] = {'IC': ic, 'Hit': hit, 'LS': ls, 'samples': n_test, 'months': n_months, 'features': n_feat, 'models': ics, 'time': elapsed, 'stocks': len(codes)}
    print(f'  IC={ic:+.4f} Hit={hit:.3f} LS={ls:+.4f} N={n_test:,} ({elapsed:.0f}s)', flush=True)

conn.close()

# ======== REPORT ========
print(); print('='*65)
print('FULL IC REPORT — All Frequencies')
print('='*65)
print(f'  {"Frequency":<12s} {"Stocks":>8s} {"Features":>8s} {"CS_IC":>8s} {"Hit":>8s} {"Top20LS":>8s} {"N_test":>10s} {"Months":>6s}')
print(f'  {"-"*70}')
for freq in ['Monthly','Weekly','Daily']:
    r = results[freq]
    print(f'  {freq:<12s} {r["stocks"]:>8,} {r["features"]:>8d} {r["IC"]:+8.4f} {r["Hit"]:8.3f} {r["LS"]:+8.4f} {r["samples"]:>10,} {r["months"]:>6d}')
print()
for freq in ['Monthly','Weekly','Daily']:
    r = results[freq]
    models_str = ' '.join([f'{n}={v:+.4f}' for n,v in sorted(r['models'].items(), key=lambda x:-x[1])])
    print(f'  {freq}: {models_str}')
print('='*65)
