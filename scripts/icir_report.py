# ICIR Report: CS_IC, ICIR, RankIC for all 3 frequencies
...
def cs_ic_ir(pred,true,dates):
    ics=np.array([spearmanr(pred[dates==m],true[dates==m])[0] for m in np.unique(dates) if (dates==m).sum()>=20])
    ic=np.mean(ics); icir=ic/np.std(ics) if np.std(ics)>0 else 0
    rank_ic=np.mean([spearmanr(np.argsort(np.argsort(pred[dates==m])),np.argsort(np.argsort(true[dates==m])))[0] for m in np.unique(dates) if (dates==m).sum()>=20])
...
for table,label in [('monthly_klines','Monthly'),('weekly_klines','Weekly'),('daily_klines','Daily')]:
    ...
    print(f'  {len(codes)} stocks, {len(flat):,} samp, {flat.shape[1]}d ({time.time()-t0:.0f}s)', flush=True)
    ...
    for n,p in [('LGB',p_lgb),('XGB',p_xgb),('Ridge',p_ridge),('MLP',p_mlp)]:
        ics[n]=max(cs_ic_ir(p,y_te,te_dates)[0],0)
    w_sum=sum(ics.values())
    p_ens=sum(ics[n]*p for n,p in [('LGB',p_lgb),('XGB',p_xgb),('Ridge',p_ridge),('MLP',p_mlp)])/w_sum
    ic,icir,rank_ic,ics_arr=cs_ic_ir(p_ens,y_te,te_dates)
    hit=np.mean((p_ens>0)==(y_te>0))

    results.append({'freq':label,'IC':ic,'ICIR':icir,'RankIC':rank_ic,'Hit':hit,'IC>0':np.mean(ics_arr>0),'ICstd':np.std(ics_arr),'months':len(ics_arr),'stocks':len(codes),'samples':len(y_te)})
    print(f'  IC={ic:+.4f} ICIR={icir:+.3f} RankIC={rank_ic:+.4f} Hit={hit:.3f} IC>0={np.mean(ics_arr>0):.0%}', flush=True)

conn.close()

print(); print('='*75)
print(f'{"Freq":<10s} {"Stocks":>6s} {"Samples":>10s} {"Months":>6s} {"IC":>8s} {"ICIR":>8s} {"RankIC":>8s} {"Hit":>8s} {"IC>0":>8s} {"ICstd":>8s}')
print('-'*75)
for r in results:
    print(f'{r["freq"]:<10s} {r["stocks"]:>6,} {r["samples"]:>10,} {r["months"]:>6d} {r["IC"]:+8.4f} {r["ICIR"]:+8.3f} {r["RankIC"]:+8.4f} {r["Hit"]:8.3f} {r["IC>0"]:7.0%} {r["ICstd"]:8.4f}')
print('='*75)
