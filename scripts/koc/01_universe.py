...
PIT status per filter (dry-run mode):
  ...
  Filter 3        : PROXY — current ST list, not historical (conservative approximation)
  Filter 4        : PROXY — SZ only (float shares x current price); SH = unknown (pass)
...
def init_out_db(conn: sqlite3.Connection) -> None:
    conn.executescript("""
        ...
        CREATE TABLE universe_liquid (
            ...
            pass_mktcap     INTEGER NOT NULL,  -- -1 = unknown (SH in dry-run)
...
def build_pools(
    ...
    # Filter 4: market cap >= 5 billion (SZ proxy; SH = -1 unknown)
    ...
    df_liquid = df[[
        ...
        "pass_mktcap", "pass_turnover", "pass_listing", "in_liquid",
    ]].copy()
...
def generate_report(
    ...
    # SH stock count in liquid pool
    sh_obs = int(df_liquid["code"].str.startswith("sh").sum())
...
| Market cap >= 5亿 | ⚠️ Proxy (SZ) | SZ: current float shares x current price; SH ({sh_obs:,} obs) = unknown = pass |
...
2. **Market cap for SH stocks** is unknown ({sh_obs:,} observations, treated as pass).
