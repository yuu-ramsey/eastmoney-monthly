"""LSTM final: train on ALL stocks, Triple-Barrier, full v2-24tp coverage."""
...
d_df = pd.read_sql_query(f"SELECT code,date,close,open,high,low,volume FROM daily_klines WHERE code IN ({','.join('?'*len(codes))}) AND date>='2010-01-01' ORDER BY code,date", conn, params=codes)
conn.close()
print(f"Daily: {len(d_df)} rows, {d_df['code'].nunique()} codes")

LOOKBACK, FWD_DAYS = 60, 126
...
for code, grp in d_df.groupby('code'):
    ...
    feats[:,0]=ret; r5=np.zeros(n); r5[5:]=np.clip((c[5:]-c[:-5])/np.maximum(c[:-5],0.01),-0.5,0.5)
    feats[:,1]=r5; r20=np.zeros(n); r20[20:]=np.clip((c[20:]-c[:-20])/np.maximum(c[:-20],0.01),-1,1)
    feats[:,2]=r20
...
print(f"Seqs: {len(X)}")
for i,nm in enumerate(['neutral','bull','bear']):
    print(f"  {nm}: {sum(1 for l in y if l==i)} ({100*sum(1 for l in y if l==i)/len(y):.1f}%)")
...
class GRU(nn.Module):
    def __init__(self): super().__init__(); self.gru=nn.GRU(10,32,1,batch_first=True); self.head=nn.Linear(32,3)
...
# Smoke
model=GRU().to(DEVICE); opt=torch.optim.AdamW(model.parameters(),lr=1e-4,weight_decay=1e-5)
model.train()
for Xb,yb in ld_tr:
    Xb,yb=Xb.to(DEVICE),yb.to(DEVICE); opt.zero_grad(); loss=loss_fn(model(Xb),yb)
...
cd={i:sum(1 for l in y_va if l==i)/len(y_va)*100 for i in range(3)}
print(f"\nSmoke: ba={ba:.4f} vs maj={maj_ba:.4f}")
print(f"CM:\n{cm}")
print(f"Class%: neutral={cd[0]:.1f} bull={cd[1]:.1f} bear={cd[2]:.1f}")
...
for ep in range(1,25):
    ...
    with torch.no_grad():
        for Xb,yb in ld_va: p=model(Xb.to(DEVICE)); preds.append(p.argmax(dim=1).cpu().numpy()); targs.append(yb.cpu().numpy())
    preds=np.concatenate(preds); targs=np.concatenate(targs)
    ba=balanced_accuracy_score(targs,preds); f1=f1_score(targs,preds,average='macro'); sched.step(ba)
    if ep<=3 or ep%5==0: print(f"Ep{ep:3d}: ba={ba:.4f} f1={f1:.4f}")
    if ba>best_ba: best_ba,best_ep,ni=ba,ep,0; torch.save(model.state_dict(),str(PROJECT/'.eastmoney-ai'/'lstm'/'lstm_final.pt'))
    ni+=1
...
# Predict 24tp
print("\nPredicting 24tp...")
model.load_state_dict(torch.load(str(PROJECT/'.eastmoney-ai'/'lstm'/'lstm_final.pt'),map_location=DEVICE)); model.eval()
...
print(f"LSTM-Final: {len(lp)} preds, {(time.time()-t0):.0f}s")
out=json.load(open(PROJECT/'data'/'p3-kronos-lstm-signals.json')); out['lstm_final']=lp
