...
// 茅台最近5日 close: 1361.33, 1354.55, 1344.09, 1342.17, 1332.95
const MAOTAI_5 = [1361.33, 1354.55, 1344.09, 1342.17, 1332.95];

test('sma: 茅台 MA5 手工验证', () => {
...
test('macd: 50 根数据 MACD 有值', () => {
  // MACD(12,26,9) 需要 26 + 9 = 35 根才出 DEA
  const closes = [];
  for (let i = 0; i < 50; i++) closes.push(100 + Math.sin(i * 0.5) * 10);
  const r = macd(closes);
  assert.ok(r.dif.some(v => v != null), 'DIF 应有值');
  assert.ok(r.dea.some(v => v != null), 'DEA 应有值');
  assert.ok(r.hist.some(v => v != null), 'HIST 应有值');
});

test('macd: < slow 全 null', () => {
